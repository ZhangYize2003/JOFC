...
def run_keypress_control(exitFlag=_EXIT_EVENT):
    ...
    turn_angle = "5"     # Turn 6 degrees each time a/d is pressed
    ...
    speed = power_map['3']  # Default to "100% power"
    turn_angle  = angle_map['4'] # Default to "10 degrees"
        ...
        if sent_packet:
            ...
            if packet is not None:
                ...
                if (
                    ...
                    print("Received acknowledgement, waiting for movement acknowledgement")

                #check for movement acknowledgement packet
                packet1 = receivePacket()
                if packet1 is not None:
                    packetType = packet1.packetType
                    res_type = packet1.command
                    if (
                            packetType == TPacketType.PACKET_TYPE_RESPONSE.value and
                            res_type == TResponseType.RESP_OK.value
                    ):
                        print("Received movement acknowledgement")
                else:
                    print("No movement acknowledgement from Arduino")
            else:
                print("No response from Arduino(1st packet)")
