...
ack_received = True # Flag to indicate if an acknowledgment has been received from the Arduino.
...
def receivePacket(exitFlag: Event = _EXIT_EVENT):
    ...
    """
    global ack_received  # Use the global ack_received flag
    ...
    while not exitFlag.is_set():
        ...
        if buffer_size == target_packet_size:
            ...
            if res_status == TResultType.PACKET_OK:
                packet = TPacket.from_buffer(payload)

                # Check if the packet is an acknowledgment (OK response)
                if packet.packetType == TPacketType.PACKET_TYPE_RESPONSE.value and packet.command == TResponseType.RESP_OK.value:
                    ack_received = True  # Set acknowledgment flag
                    print("Acknowledgment received: OK")
                else:
                    ack_received = False  # Reset acknowledgment flag if not OK
                    print("Error: Received non-OK response")

                return packet
            else:
                handleError(res_status)
                ack_received = False  # Reset acknowledgment flag on error
                return None
...
def sendPacket(packetType: TPacketType, commandType: TCommandType, params: []):
    ...
    # Prevent sending a new packet if the previous one is not acknowledged
    if not ack_received:
        print("Cannot send packet: Previous packet not acknowledged.")
        return

    # Reset acknowledgment flag before sending a new packet
    ack_received = False
    ...
    # Pad params with zeros if fewer than expected parameters
    if len(params) < PAYLOAD_PARAMS_COUNT:
        params = params + [0] * (PAYLOAD_PARAMS_COUNT - len(params))

    # Debug: Print the full packet details
    print("Sending Packet:")
    print(f"  Packet Type: {packetType.name}")
    print(f"  Command: {commandType.name}")
    print(f"  Params (after padding): {params}")
...
def run_keypress_control(exitFlag=_EXIT_EVENT):
    ...
    while not exitFlag.is_set():
        ...
        # Move forward (w) or backward (s) with fixed distance
            ...
            sendPacket(TPacketType.PACKET_TYPE_COMMAND, TCommandType.COMMAND_FORWARD, [move_distance, speed])

        elif key == 's':
            print(f"Move backward {move_distance} cm at {speed}% power")
            sendPacket(TPacketType.PACKET_TYPE_COMMAND, TCommandType.COMMAND_REVERSE, [move_distance, speed])

        # Turn left (a) or right (d) with fixed angle
        elif key == 'a':
            print(f"Turn left {turn_angle} degrees at {speed}% power")
            sendPacket(TPacketType.PACKET_TYPE_COMMAND, TCommandType.COMMAND_TURN_LEFT, [turn_angle, speed])

        elif key == 'd':
            print(f"Turn right {turn_angle} degrees at {speed}% power")
            sendPacket(TPacketType.PACKET_TYPE_COMMAND, TCommandType.COMMAND_TURN_RIGHT, [turn_angle, speed])
